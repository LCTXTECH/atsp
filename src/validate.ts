...
export function validateDeclaration(
  ...
  if (typeof declaration.amount !== 'number' || declaration.amount <= 0) {
    ...
    if (declaration.amount > 1_000_000) {
      riskScore += 0.5
      warnings.push(`amount ${declaration.amount} is very large — verify units`)
  ...
  if (declaration.slippage !== undefined) {
    if (
      typeof declaration.slippage !== 'number' ||
      declaration.slippage < 0 ||
      declaration.slippage > 100
    ) {
  ...
  if (!declaration.decisionTrace) {
    ...
    if (
      !declaration.decisionTrace.reasoning ||
      declaration.decisionTrace.reasoning.trim().length < 10
    ) {
    ...
    if (
      ...
      riskScore += 0.15
      warnings.push(
        `agent confidence is ${declaration.decisionTrace.confidence} — low confidence increases risk`
      )
    }
    ...
    if (
      !declaration.decisionTrace.input ||
      Object.keys(declaration.decisionTrace.input).length === 0
    ) {
...
export function isExpired(
  declaration: ATSPIntentDeclaration,
  ttlMs: number = DECLARATION_TTL_MS
): boolean {
